...
// Load EasyMDE resources dynamically
async function loadEditorResources() {
  ...
  css.rel = 'stylesheet';
  css.href = 'https://unpkg.com/easymde/dist/easymde.min.css';
...
// Custom image handler for EasyMDE
...
async function openFile(filePath) {
  try {
    ...
    if (data.success) {
      ...
      editor.value(currentFile.markdown || '');
...
function renderCollageView(files) {
  ...
  // Sync toggle state and apply class
  const toggle = document.getElementById('view-mode-toggle');
  if (toggle) toggle.checked = currentViewMode === 'list';
